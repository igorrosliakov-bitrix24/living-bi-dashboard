...
export class FileDashboardStore extends DashboardStore {
  ...
  async load() {
    try {
      ...
      if (snapshot?.format !== 1 && snapshot?.format !== 2 && snapshot?.format !== 3) {
        throw new Error("Unsupported dashboard state format.");
      }

      this.replaceVersions(snapshot.versions, snapshot.currentVersion);
  ...
  async reset() {
    return this.persistAfterChange(() => super.reset());
  }
  ...
  async persistAfterChange(change) {
    ...
    try {
      ...
      this.replaceVersions(before.versions, before.currentVersion);

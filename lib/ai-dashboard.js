...
export function createDevelopmentRequestCompletion(command) {
  const normalizedCommand = normalizeCommand(command);
    ...
    messages: [
      {
        role: "system",
        content: [
          "Ты формируешь заявку для агента разработки BI-дашборда.",
          "Текущая версия дашборда не смогла безопасно выполнить запрос пользователя.",
          "Вызови request_development и конкретно опиши недостающие возможности, ожидаемую работу и проверяемые критерии готовности.",
          "Не добавляй код, ключи, токены, карточки CRM или персональные данные."
        ].join(" ")
      },
      { role: "user", content: normalizedCommand }
    ],
    tools: [createDevelopmentRequestTool()],
    tool_choice: { type: "function", function: { name: "request_development" } }
  };
}
...
export function createVisualCommandProposal(command, dashboard) {
  const normalizedCommand = normalizeCommand(command);
  const lowerCaseCommand = normalizedCommand.toLocaleLowerCase("ru-RU");

  if (/(добавь|новый виджет|конверси|клиент|воронк|исключ|период|квартал|месяц|задач|отдел|недел|динамик)/u.test(lowerCaseCommand)) {
    return null;
  }

  const barIndex = dashboard.widgets.findIndex((widget) => widget.type === "bar");
  if (barIndex < 0) {
    return null;
  }
  ...
  if (/менеджер/u.test(lowerCaseCommand)) {
    patch.push({ op: "replace", path: `/widgets/${barIndex}/groupBy`, value: ["assignedById"] });
    changes.push("группировку по менеджерам");
  }

  if (/горизонталь/u.test(lowerCaseCommand)) {
    patch.push({ op: "replace", path: `/widgets/${barIndex}/options/orientation`, value: "horizontal" });
    changes.push("горизонтальную ориентацию");
  }

  if (/цветах бренда|цвета бренда|бренд|битрикс24/u.test(lowerCaseCommand)) {
    patch.push({ op: "add", path: `/widgets/${barIndex}/options/palette`, value: "bitrix24" });
    changes.push("палитру Битрикс24");
  }

  if (/по возрастанию/u.test(lowerCaseCommand)) {
    patch.push({ op: "replace", path: `/widgets/${barIndex}/options/sort`, value: "asc" });
    changes.push("сортировку по возрастанию");
  } else if (/по убыванию/u.test(lowerCaseCommand)) {
    patch.push({ op: "replace", path: `/widgets/${barIndex}/options/sort`, value: "desc" });
    changes.push("сортировку по убыванию");
  }
  ...
  const kpiNote = /kpi|кпи/u.test(lowerCaseCommand) ? " KPI-карточки остаются над графиками." : "";
  return createProposalFromPatch(dashboard, patch, `Подготовлены ${changes.join(", ")}.${kpiNote}`);
}

export function createConversionCommandProposal(command, dashboard) {
  const normalizedCommand = normalizeCommand(command);
  const lowerCaseCommand = normalizedCommand.toLocaleLowerCase("ru-RU");

  if (!/конверси/u.test(lowerCaseCommand) || !/(нов(ым|ые) клиент|isreturning)/u.test(lowerCaseCommand) || !/(тест|воронк)/u.test(lowerCaseCommand) || !/(квартал|this_quarter)/u.test(lowerCaseCommand)) {
    return null;
  }

  const next = structuredClone(dashboard);
  const bar = next.widgets.find((widget) => widget.type === "bar" && widget.entity === "deals");
  const baseFilter = { isReturning: false };

  next.period = { field: "closedAt", preset: "this_quarter" };

  if (bar) {
    bar.filter = baseFilter;
    bar.categoryExclusions = ["Тест"];
  }

  const removeIds = new Set(["new-client-deals", "new-client-won", "new-client-conversion"]);
  next.widgets = next.widgets.filter((widget) => !removeIds.has(widget.id));
  next.widgets.push(
    {
      id: "new-client-deals",
      type: "kpi",
      title: "Новые клиенты в воронках",
      entity: "deals",
      filter: baseFilter,
    ...
    },
    {
      id: "new-client-won",
      type: "kpi",
      title: "Успешные сделки новых клиентов",
      entity: "deals",
      filter: { ...baseFilter, stageSemanticId: "S" },
      categoryExclusions: ["Тест"],
      aggregate: { fn: "count" }
    },
    {
      id: "new-client-conversion",
      type: "kpi",
      title: "Конверсия новых клиентов",
      computed: { expr: "new-client-won / new-client-deals", format: "percent" }
    }
  );

  return {
    dashboard: next,
    summary: "Подготовлены конверсия новых клиентов, исключение воронки «Тест» и период текущего квартала."
  };
}
...
export function createDevelopmentFallback(command) {
  return createDevelopmentRequest(command, {
    title: "Доработка BI-дашборда по запросу пользователя",
    reason: "ИИ-редактор не смог собрать безопасное изменение в рамках текущей спецификации дашборда. Для этого запроса нужна реализация новой возможности или явного бизнес-правила.",
    requiredCapabilities: [
      "Проанализировать данные и поля портала, необходимые для исходного запроса.",
      "Расширить безопасную JSON-спецификацию дашборда нужными настройками, виджетами или правилами расчёта.",
      "Добавить серверную проверку, preview агрегатов и diff перед применением изменений."
    ],
    expectedResult: [
      "Пользователь вводит исходную фразу и получает понятный черновик изменений.",
      "После подтверждения дашборд показывает результат на данных портала.",
      "История версий позволяет вернуться к предыдущей конфигурации дашборда."
    ],
    acceptanceCriteria: [
      "Запрос проходит на тестовых данных портала без изменения CRM-записей.",
      "Перед сохранением пользователь видит preview и список изменений.",
      "Новая конфигурация сохраняется отдельной версией и поддерживает возврат к прошлой версии."
    ]
  });
}
...
function createDashboardTools() {
  return [
    ...
    createDevelopmentRequestTool()
  ];
}

function createDevelopmentRequestTool() {
  return tool("request_development", "Подготовить структурированную заявку, когда текущих возможностей дашборда недостаточно.", {
    type: "object",
    properties: {
      title: { type: "string" },
      reason: { type: "string" },
...
export function buildDashboardDiff(current, next) {
  ...
  if (JSON.stringify(current.period) !== JSON.stringify(next.period)) {
    changes.push(`Период данных: ${periodLabel(next.period?.preset)}.`);
  }
  ...
  for (const [id, nextWidget] of nextWidgets) {
    ...
    if (JSON.stringify(currentWidget.filter) !== JSON.stringify(nextWidget.filter) || JSON.stringify(currentWidget.categoryExclusions) !== JSON.stringify(nextWidget.categoryExclusions)) {
      changes.push(`Изменены фильтры «${nextWidget.title}».`);
    }

...
export class ConnectorStatusStore {
  ...
  async recordError(code = "request_failed", { table } = {}) {
    const now = new Date().toISOString();
    const datasets = { ...this.state.datasets };
    // Отказ по конкретной таблице записывается рядом с её успехами, иначе
    // по статусу не понять, какой именно набор перестал обслуживаться.
    if (table) datasets[table] = { ...(datasets[table] || {}), lastRequestAt: now, lastErrorAt: now, lastErrorCode: code, requests: (datasets[table]?.requests || 0) + 1 };
    this.state = { ...this.state, lastRequestAt: now, lastErrorAt: now, lastErrorCode: code, lastTable: table || this.state.lastTable, requests: this.state.requests + 1, datasets };
    await this.save();
  }
